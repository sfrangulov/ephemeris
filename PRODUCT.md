...
Daily npm download counts and GitHub stargazer counts for a maintainer's package portfolio, with weekly momentum deltas (up / flat / down) across 13 weeks. Watchlists are auto-derived from npm (where the maintainer's GitHub login is also their npm publisher name); each maintainer gets a public `/u/<slug>` portfolio page and an embeddable `/badge/<slug>` SVG. The distribution loop is: maintainer signs in → portfolio populates → drops the badge in repo READMEs → README readers click through → land on the observatory → some sign in for their own.
...
6. **Dark-first; badges adapt.** The web app is dark-only (no theme switcher — the product is meant for low-light glances). The embeddable badge SVG carries both palettes inside one URL via `prefers-color-scheme` so README readers see the right rendering automatically.
