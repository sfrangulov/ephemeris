...
| `Badge SVG · per-package` | `app/badge/[slug]/[...rest]/route.ts` + `lib/badge-pkg.ts` | Four variants under one handler: `momentum.svg` / `sparkline.svg` / `stars.svg` / `card.svg`. Catch-all `[...rest]` collects pkg segments (1 for `cron`, 2 for `@scope/name`) + the final variant filename. Watchlist-gated — any miss → a cacheable 404 (`badgeNotFound()`, byte-identical 1×1 SVG so it never confirms slug existence; `CDN-Cache-Control s-maxage=60` so renamed/typo'd README URLs stop hammering origin). Three micros are 20px-tall square strips (no border-radius), brand dot at cx=10, JetBrains Mono throughout; card is 440×120 of the same vocabulary. `?theme=dark` (default) / `light` / `auto` — `auto` is the only mode that emits the `<style>` media-query flip; the other two lock to one palette. Both badge routes: `Cache-Control public, max-age=300, stale-while-revalidate=3600` + `CDN-Cache-Control public, s-maxage=3600, stale-while-revalidate=3600` — the 1h edge fresh window caps how stale the render-time freshness pill can read (honest-data invariant), well inside the 6h sync cadence. |
...
- **`deltaStars` (week-over-week stars)** is `weeklyStarDelta` — the sum of daily `stars_delta` over the trailing 7 days, NOT the most recent `star_daily` row's delta (a single row conflates ingestion timing with measurement; see the `download_daily` note below). One shared helper in `lib/aggregate.ts` feeds both the matrix `★` column and the per-package badge so they cannot drift.
